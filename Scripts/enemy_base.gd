...
func _ready() -> void:
	...
	lighs_on_the_level = get_tree().get_nodes_in_group("Lights")
...
func _physics_process(delta: float) -> void:
	...
	model_manager.enemy_is_chasing = is_chasing
	if is_chasing:
...
func checkLightsAreOn():
	if not lighs_on_the_level:
		print(name, " NOT ABLE TO FIND LIGHTS TO REACT TO")
		return
	var closest_light: Node3D = null
	var closest_distance_to_player = 8
	for light in lighs_on_the_level:
		if not light:
			continue
	...
	if closest_light:
		spotlight_ai.visible = not closest_light.is_visible_in_tree()
