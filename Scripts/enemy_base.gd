...
@export var path_ai: Path3D = null
@export var to_fix_object_ai: BaseLevelObject = null
...
func _physics_process(delta: float) -> void:
	...
	if catchPlayer_area.is_there_collision:
		...
		model_manager.enemy_state = state# = true
	...
	var ray_collider = raycast_sight.get_collider()
	...
	if not ray_collider is Player:
		if state == states.CHASING and $ChasingTimer.is_stopped():
			$ChasingTimer.start()
	if state == states.CHASING:
		...
		if velocity.length() >= speed*fixing_broken_multiplier_speed - PI/2:
			...
			if not global_position.is_equal_approx(position + vector_look_at):
				linesight_area.get_parent_node_3d().look_at(position + vector_look_at, Vector3.UP, true)
		debug_label.text += str("\n FixObject: ", to_fix_object_ai.name)
	else:
